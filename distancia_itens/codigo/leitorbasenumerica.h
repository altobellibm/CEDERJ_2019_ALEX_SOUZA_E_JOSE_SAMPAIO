...
public:
    BASE_NUM(){}
     ...
     Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic> getMatrix(){
        ...
        if (m_transacoes.empty()) return Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic>(0,0);

        int subjects = m_transacoes.size();
        int stimuli = m_cabecalho.size();

        Eigen::Matrix<int, Eigen::Dynamic, Eigen::Dynamic> dynamic_size_matrix(subjects, stimuli);
